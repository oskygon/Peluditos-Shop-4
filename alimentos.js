...
function inicializar() {
  fetch('https://backendPeluditos.pythonanywhere.com/productos')
    .then(response => response.json())
    .then(datos => {
      const contenidoPerro = document.getElementById("contenido-alimento-perro");
      const contenidoGato = document.getElementById("contenido-alimento-gato");
      const contenidoAccesorioGato = document.getElementById("contenido-accesorio-gato");
      const contenidoAccesorioPerro = document.getElementById("contenido-accesorio-perro");

      if (contenidoPerro) {
        const alimentosPerro = datos.filter(producto => producto.categoria === 'alimento para perros');
        renderizar(contenidoPerro, grupos(alimentosPerro, 4));
      }
      if (contenidoGato) {
        const alimentosGato = datos.filter(producto => producto.categoria === 'alimento para gatos');
        renderizar(contenidoGato, grupos(alimentosGato, 4));
      }
      if (contenidoAccesorioGato) {
        const accesoriosGato = datos.filter(producto => producto.categoria === 'accesorios para gatos');
        renderizar(contenidoAccesorioGato, grupos(accesoriosGato, 4));
      }
      if (contenidoAccesorioPerro) {
        const accesoriosPerro = datos.filter(producto => producto.categoria === 'accesorios para perros');
        renderizar(contenidoAccesorioPerro, grupos(accesoriosPerro, 4));
      }
    })
